...
import { rawFind, rawFindOne, toOidFilter } from '@/lib/rawMongo';
...
async function resolveStudent(query = {}) {
  ...
  if (email) {
    ...
    if (user && user.studentProfile) {
      return {
        _id: user.studentProfile.id,
        name: `${user.studentProfile.firstName} ${user.studentProfile.lastName}`,
        rollNumber: user.studentProfile.rollNumber || user.studentProfile.indexNumber,
        email: user.email,
  ...
  if (studentId) {
    // If studentId is an ObjectId format, try matching as userId first, then as profile id
  ...
  // Fallback to rawMongo (Mongoose collection fallback)
  if (studentId) {
    if (!mongoose.Types.ObjectId.isValid(studentId)) {
      return null;
    }
    return rawFindOne('students', { _id: toOidFilter(studentId) });
  }

  if (rollNumber) {
    return rawFindOne('students', { rollNumber });
  }

  if (email) {
    return rawFindOne('students', { email });
  }
...
export async function getStudentDashboardData(query = {}) {
  ...
  const results = await rawFind('Result', { student: toOidFilter(student._id) }, {
    sort: { createdAt: -1 }
  });
