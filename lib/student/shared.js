...
import { rawFindOne, toOidFilter } from '@/lib/rawMongo';
...
export async function resolveUserForStudent(student) {
  ...
  const prismaUser = await prisma.user.findUnique({ where: { email: student.email } });
  if (prismaUser) {
    return prismaUser;
  }

  return rawFindOne('User', { email: student.email, role: 'Student' });
}
